...
- `/analise/[slug]` — server component que `SELECT WHERE slug=$1 AND status='publicada'`, lê `conteudo` como `AnaliseGeradaConteudo` (não `AnaliseData`). **Não usa `PageProposta`** — monta seu próprio `<main>` enxuto, sem Header e sem Footer. `noindex` via robots meta (slug é o token).
