...
import {
  enviarEmailQualificacao,
  type QualificacaoAnswer,
} from "../../../../lib/email/qualificacao";

const SITE_ID = "augustofelipe";
const SOURCE = "creator-elite-qualificacao";
...
function sanitizeAnswers(raw: unknown): QualificacaoAnswer[] {
  if (!Array.isArray(raw)) return [];
  const out: QualificacaoAnswer[] = [];
...
export async function POST(req: Request) {
  ...
  if (!token || token.length < 16) {
    console.error("[creator-elite/qualificacao] LEADS_INGEST_TOKEN nao configurado");
  ...
  try {
    ...
    console.error("[creator-elite/qualificacao] ingest fetch failed", err);
  ...
  after(async () => {
    try {
      ...
      console.error("[creator-elite/qualificacao] notify exception", err);
    }
  });
