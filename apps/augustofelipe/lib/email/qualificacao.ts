...
export interface QualificacaoAnswer {
  step: string;
  question: string;
  value: string;
}

export interface QualificacaoEmailArgs {
  to: string;
  lead: {
    name: string;
    email: string;
...
function buildHtml({ lead, answers, submittedAt }: QualificacaoEmailArgs): string {
...
<head>
  ...
  <title>Nova qualificação Creator Elite</title>
</head>
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width: 720px; background: #fff; border-radius: 20px; overflow: hidden;">
          <tr>
            <td style="padding: 32px 32px 16px;">
              <p style="margin: 0; font-size: 12px; color: #6b7280; letter-spacing: 0.08em; text-transform: uppercase; font-weight: 600;">Creator Elite · Qualificação</p>
              <h1 style="margin: 8px 0 4px; font-size: 24px; font-weight: 700; line-height: 1.25; color: #111827;">
                Nova qualificação preenchida por ${safeName}
              </h1>
...
function buildText({ lead, answers, submittedAt }: QualificacaoEmailArgs): string {
  const lines = [
    `Nova qualificação Creator Elite preenchida por ${lead.name}.`,
...
export async function enviarEmailQualificacao(
  args: QualificacaoEmailArgs,
): Promise<SendResult> {
  const client = getClient();
  if (!client) {
    console.warn(
      "[email/qualificacao] RESEND_API_KEY ausente — pulando notificacao para",
      args.to,
    );
  ...
  const subject = `[Creator Elite] Nova qualificação — ${args.lead.name}`;
  ...
  try {
    ...
    if (error) {
      console.error("[email/qualificacao] resend falhou", error);
      return { ok: false, error: error.message ?? "unknown" };
    }
    return { ok: true, id: data?.id };
  } catch (err) {
    console.error("[email/qualificacao] exception", err);
